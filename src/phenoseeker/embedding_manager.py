...
class EmbeddingManager:
    ...
    def load(
        ...
        Args:
            ...
            embeddings_file (str | Path | np.ndarray): Path to the .npy embeddings file
                or a numpy array.
            metadata_file (str | Path | None): Path to the metadata file. Supports
                .parquet, .csv, and .csv.gz. If None, uses self.df.
        ...
        if metadata_file is not None:
            ...
            suff = "".join(metadata_path.suffixes).lower()  # handles .csv.gz
            if suff.endswith(".parquet"):
                loaded_metadata = pd.read_parquet(metadata_path)
            elif suff.endswith(".csv") or suff.endswith(".csv.gz"):
                loaded_metadata = pd.read_csv(metadata_path)  # add sep=... if needed
            else:
                raise ValueError(
                    f"Unsupported metadata format for {metadata_path.name}. "
                    "Supported: .parquet, .csv, .csv.gz"
                )
        else:
        ...
        indices = self.df["_order_key"].map(order_map).astype(int).to_numpy(dtype=int)
    ...
    def compute_features_stats(
        ...
        def calculate_and_format_stats(subset_indices, plate_label):
            ...
            control_indices = self.df.loc[subset_indices, "Metadata_Is_dmso"]
        ...
        all_stats_df = pd.concat(
            [global_stats_df] + list(plate_stats_dfs), ignore_index=True
        )
    ...
    def grouped_embeddings(
        ...
        custom_group_by_columns: list[str] | None = None,
        ...
        Args:
            group_by (str): The entity to group by ('well' or 'compound' or 'custom').
            ...
            cols_to_keep (list[str], optional): Columns to keep in the resulting
                DataFrame.
            custom_group_by_columns (list[str]): If group_by is 'custom', list of cols
                used to group by embeddings
        ...
        if group_by in ["image", "well"]:
            ...
            if custom_group_by_columns is None:
                raise ValueError(
                    f"Group by '{group_by}' requires 'custom_group_by_columns' columns"
                )
            if cols_to_keep is None:
                raise ValueError(
                    f"Group by '{group_by}' requires 'cols_to_keep' columns"
                )
            group_by_columns = custom_group_by_columns
        else:
            raise ValueError(
    ...
    def apply_spherizing_transform(
        ...
        def sphering_plate(plate_indices):
            ...
            spherize = Spherize(epsilon=1e-4, center=True, method=method)
